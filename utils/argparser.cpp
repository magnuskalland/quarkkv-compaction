...
static const char *usage = R"(usage %s [OPTIONS]
    ...
    -f, --sst-file-size             SST file size               (Default %d MiB)
...
void parse_args(int argc, char *argv[], Config *dest)
{
    ...
    while ((opt = getopt_long(argc, argv, "e:m:d:w:l:s:k:z:f:h:c:", long_options,
                              nullptr)) != -1) {
        switch (opt) {
            case 'e':
                engine = true;
...
parse_args_err:
    fprintf(stderr, usage, argv[0], DEFAULT_LEVELS, DEFAULT_FANOUT, DEFAULT_KEY_SIZE,
            DEFAULT_LEVEL0_MAX_SIZE, DEFAULT_SST_FILE_SIZE >> 20, "one");
    exit(EXIT_FAILURE);
}
